...
def f_(fov):
    ...
    dec.fov = fov
    save_folderf = r'Z:\MERFISHp\12_16_2025_BigSlideTest\decoded_files_th3'
...
def f(fov):
    ...
    return fov,None
def run_in_batches(values, batch_size=10, workers=10):
    for i in range(0, len(values), batch_size):
        batch = values[i:i + batch_size]
        print(f"Running batch {i//batch_size + 1}: {batch}")

        with ProcessPoolExecutor(max_workers=workers) as ex:
            futures = [ex.submit(f, v) for v in batch]
            for fut in as_completed(futures):
                convx, out = fut.result()
                print("done:", convx, "->", out)
...
if __name__ == "__main__":
    ...
    fovs = np.sort([fov for fov in dic_pos])
    fovs = list(fovs[560:])+list(fovs[:560])
    
    #iset = 1
    #fovs = np.sort([fov for fov in dic_pos if f'zscan{iset}_' in fov])
    
    run_in_batches(fovs[:], batch_size=10, workers=10)
